...
Version 3 -- updated March 13, 2024
...
Added batch processing: Input numbers can now automatically be parsed from a
simple input.txt of digits separated in lines:
1
2
3
CLI input must be none with n-bit carry-save per default unless an input number and
[n-bit|fast] [carry-save|wallace|recursive] are specified in CLI.
...
-- Process each number with given settings
processNumbers :: [Integer] -> String -> String -> IO ()
processNumbers [] _ _ = return ()
processNumbers (n:ns) adderType multType = do
  let outputFileName = "" ++ convertNumberForFilename n ++ ".dimacs"
      output_bits = int2bits n
      adder = case adderType of
                "n-bit" -> n_adder
                "fast" -> fast_adder
                _ -> error ("Adder " ++ adderType ++ " unknown")
      multiplier = sym_multiplier
                     (case multType of
                      ...
                      _ -> error ("Multiplier " ++ multType ++ " unknown"))
      (((as,bs),cs),last,cnf) = multiplier output_bits
      cnfOutput = generateCNFOutput n adderType multType as bs cs last cnf

  -- Output to file
  writeFile outputFileName cnfOutput
  putStrLn $ "Processing number: " ++ show n ++ ", written to " ++ outputFileName
  putStr "\n"

  -- Continue with next number
  processNumbers ns adderType multType

-- Helper to generate CNF output from given parameters
generateCNFOutput :: Integer -> String -> String -> [Bit] -> [Bit] -> [Bit] -> Var -> [CNFOr] -> String
generateCNFOutput n adderType multType as bs cs lastVar cnf = unlines [
    "c Problem generated from " ++ multType ++ "[" ++ adderType ++ "] multiplication circuit",
    "c by Paul Purdom and Amr Sabry",
    "c",
    "c Circuit for product = " ++ show n ++  " " ++ (show (reverse (int2bits n))),
    ...
    "c",
    "p cnf " ++ show lastVar ++ " " ++ show (length (filter cnf_pred cnf)),
...
main = do
  args <- getArgs
  if null args then
    -- Batch mode: Process numbers from "input.txt" with default settings
    batchMode
  else
    -- CLI Mode: Process a single number with specified configurations
    cliMode args

cliMode :: [String] -> IO ()
cliMode args = do
  if length args /= 3 then
    error "Usage: input number [n-bit|fast] [carry-save|wallace|recursive] or run without arguments for batch mode\n"
  else return ()

  let output_num_dec =
        let n = read (args !! 0) :: Integer in
  ...
  let cnfOutput = generateCNFOutput output_num_dec (args !! 1) (args !! 2) as bs cs last cnf
...
batchMode = do
  ...
  let defaultAdder = "n-bit"
      defaultMult = "carry-save"
  processNumbers numbers defaultAdder defaultMult
